...
class focal_loss(nn.Module):
    def __init__(self, alpha=0.75, gamma=2, num_classes = 2, size_average=True):
        ...
        if isinstance(alpha,list):
            ...
            assert alpha<10  #如果α为一个常数,则降低第一类的影响,在目标检测中为第一类
            # print("Focal_loss alpha = {} ,将对背景类进行衰减,请在目标检测任务中使用.".format(alpha))
            self.alpha = torch.zeros(num_classes)
            self.alpha[0] += 10-alpha
            self.alpha[1:] += (alpha) # α 最终为 [ α, 1-α, 1-α, 1-α, 1-α, ...] size:[num_classes]
    ...
    def forward(self, preds, labels):
        ...
        loss = torch.mul(self.alpha, loss.t()) * 2
...
class focal_loss_8(nn.Module):
    def __init__(self, alpha=0.5, gamma=2, num_classes = 8, size_average=True):
        """
        focal_loss损失函数, -α(1-yi)**γ *ce_loss(xi,yi)
        步骤详细的实现了 focal_loss损失函数.
        :param alpha: 阿尔法α,类别权重.      
        ...
        :param size_average: 损失计算方式,默认取均值
        """
        super(focal_loss_8,self).__init__()
        self.size_average = size_average
        if isinstance(alpha,list):
            assert len(alpha)==num_classes   
            # α可以以list方式输入,
