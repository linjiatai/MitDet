...
class RandomApply_imgs(RandomTransforms):
    """Apply randomly a list of transformations with a given probability
    Args:
        transforms (list or tuple): list of transformations
        p (float): probability
    ...
    def __init__(self, transforms, p=0.5):
        super(RandomApply_imgs, self).__init__(transforms)
        self.p = p
...
class RandomHorizontalFlip_imgs(object):
    """Horizontally flip the given PIL Image randomly with a given probability.
    Args:
        p (float): probability of the image being flipped. Default value is 0.5
    """
...
class RandomVerticalFlip_imgs(object):
    """Vertically flip the given PIL Image randomly with a given probability.
    Args:
        p (float): probability of the image being flipped. Default value is 0.5
    """
...
class ColorJitter_imgs(object):
    ...
    def get_params(brightness, contrast, saturation, hue):
        ...
        if saturation is not None:
            # torch.random.manual_seed(seed3)
            saturation_factor = random.uniform(saturation[0], saturation[1])
            transforms.append(Lambda(lambda img: F.adjust_saturation(img, saturation_factor)))

        if hue is not None:
            # torch.random.manual_seed(seed4)
            hue_factor = random.uniform(hue[0], hue[1])
            transforms.append(Lambda(lambda img: F.adjust_hue(img, hue_factor)))

        # torch.random.manual_seed(seed5)
    ...
    def __call__(self, imgs):
        ...
        # seed3 = torch.random.seed()
        # seed4 = torch.random.seed()
        # seed5 = torch.random.seed()
        # transform = self.get_params(self.brightness, self.contrast,
        #                             self.saturation, self.hue,seed1,seed2,seed3,seed4,seed5)
        transform = self.get_params(self.brightness, self.contrast,
                                    self.saturation, self.hue)
        imgs[0]=transform(imgs[0])
        imgs[1]=transform(imgs[1])
        return imgs
...
class HEDJitter_imgs(object):
    ...
    def __call__(self, imgs):
        imgs[0]=self.adjust_HED(imgs[0], self.alpha, self.betti)
        # imgs[1]=self.adjust_HED(imgs[1], self.alpha, self.betti)
        return imgs
...
class AutoRandomRotation_imgs(object):
    """auto randomly select angle 0, 90, 180 or 270 for rotating the image.
    Args:
        resample ({PIL.Image.NEAREST, PIL.Image.BILINEAR, PIL.Image.BICUBIC}, optional):
            An optional resampling filter. See `filters`_ for more information.
    ...
    def __call__(self, imgs):
        imgs[0]=F.rotate(imgs[0], self.degrees, self.resample, self.expand, self.center, self.fill)
        imgs[1]=F.rotate(imgs[1], self.degrees, self.resample, self.expand, self.center, self.fill)
...
class RandomElastic(object):
    ...
    def __init__(self, alpha, sigma):
        ...
        self.alpha = alpha
        # self.sigma = np.random.uniform(sigma,0.1)
        self.sigma =sigma
    @staticmethod
...
class RandomElastic_imgs(object):
    """Random Elastic transformation by CV2 method on image by alpha, sigma parameter.
        # you can refer to:  https://blog.csdn.net/qq_27261889/article/details/80720359
        # https://blog.csdn.net/maliang_1993/article/details/82020596
        # https://docs.scipy.org/doc/scipy/reference/generated/scipy.ndimage.map_coordinates.html#scipy.ndimage.map_coordinates
    ...
    @staticmethod
    def RandomElasticCV2(img, alpha, sigma,seed1,seed2, mask=None):
        alpha = img.shape[1] * alpha
        sigma = img.shape[1] * sigma
        if mask is not None:
            mask = np.array(mask).astype(np.uint8)
    ...
    def __call__(self, imgs, mask=None):
        ...
        seed1 = torch.random.seed()
        seed2 = torch.random.seed()
        imgs[0]=self.RandomElasticCV2(np.array(imgs[0]), self.alpha, self.sigma,seed1,seed2, mask)
        imgs[1]=self.RandomElasticCV2(np.array(imgs[1]), self.alpha, self.sigma,seed1,seed2, mask)
        return imgs
